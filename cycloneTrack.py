...
def get_cyclone_movement(data, issued_signal):
    ...
    if not issued_signal:
        ...
        track_df = pd.read_excel(f"data/typhoon/{cyclone_name}/{cyclone_name}_track.xlsx")
    ...
    print(track_df.tail())
    track_df.to_excel(f"data/typhoon/{cyclone_name}/{cyclone_name}_track.xlsx", index=False)
...
def get_cyclone_forecast(data):
    ...
    forecast_df.to_excel(f"data/typhoon/{cyclone_name}/{cyclone_name}_forecast.xlsx", index=False)
...
if __name__ == "__main__":
    ...
    if len(cyclones) > 1:
        ...
        for cyclone in cyclones:
            ...
            if data:
                cyclone_name = data["TropicalCycloneName"]
                issued = input(f"Is Tropical Cyclone Warning Signal No. 1 or above in force for {cyclone_name}? (y/n): ").lower()
                while True:
                    if issued == 'y':
        ...
        for idx, cyclone in enumerate(cyclones):
            print(cyclone)
            t = threading.Thread(target=get_cyclone_info, args=(cyclone, issued_signal[idx]))
